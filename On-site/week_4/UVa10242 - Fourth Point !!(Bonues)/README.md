...
1. **核心幾何性質**：第四點 $D = A + B - C$（其中 $C$ 為交點，$A$、$B$ 為另外兩端點）
2. **數學降維優化**：利用 4 個輸入點的總和 $\text{Sum} = A + B + 2C$，推導出 $D = \text{Sum} - 3C$ 來簡化程式碼
...
- 如果我們把這 4 個點加總：$$\text{Sum} = A + B + 2C$$

- 我們原本需要的答案是：$$A + B - C$$

- 仔細觀察可以發現，只要拿總和 $\text{Sum}$ 減去 3 個 $C$，就會剛好等於答案：
$$\text{Sum} - 3C = (A + B + 2C) - 3C = A + B - C$$
